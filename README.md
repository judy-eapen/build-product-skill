...
| `/infosec-doc` | Fill out your organization's **InfoSec Questionnaire** (the Ops/DevOps security-review `.xlsx`) for a feature. Asks for the template path at first run, then introspects the template's structure at runtime (no hardcoded tabs or cell maps). Derives answers from the PRD, system design, technical review, codebase review, diagrams, and `_pipeline-state.json`, then **batch-interviews** the PM for what no artifact carries (severity, escalation/emergency contacts, DR region, vendor SLA, approved-AI-list + opt-out, legal sign-off). **Never fabricates** a security answer — each cell is sourced, PM-confirmed, or `⚠ NEEDS INPUT`; industry-standard defaults (TLS/AES) are flagged for confirmation. Writes the real workbook via `openpyxl` (formatting + dropdowns preserved) to `infosec/[feature]-infosec-questionnaire.xlsx`. Standalone — not auto-invoked. Idempotent. |
