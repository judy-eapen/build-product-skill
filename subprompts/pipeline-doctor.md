...
| **B4** | If `current_step` is at or after a step that comes after Gate N, then `gates.gate_N` must be either `"Approved [date]"` OR (for Gate 2 only, v2.8.1+) `"Deferred — DRAFT mode (...)"`. The Deferred status is legitimate when designs aren't finalized yet — recognize it by checking that `user_stories.mode == "DRAFT"` (or `"MIXED"`) AND `artifacts.design_catalogs` is empty. If those conditions hold and gate_2 is `"Pending"`, treat it as a deferred-but-not-yet-recorded state and propose updating gate_2 to the explicit `"Deferred — DRAFT mode (...)"` string (downgrade to INFO instead of WARNING). Mismatches outside this pattern still WARN. | WARNING / INFO |
